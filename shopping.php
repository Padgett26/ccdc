    ...
    if (filter_input(INPUT_POST, 'postnote', FILTER_SANITIZE_STRING)) {
        ...
        if ($id == "new") {
            ...
            if ($delpost == "1") {
                ...
                if ($delpic1 == '1') {
                    $stmt3 = $db->prepare(
                            "SELECT pic1 FROM shopping WHERE id=?");
                    $stmt3->execute(array(
                            $id
                    ));
                    $row3 = $stmt3->fetch();
                    ...
                    $stmt4 = $db->prepare(
                            "UPDATE shopping SET pic1=? WHERE id=?");
                    $stmt4->execute(array(
                            '0',
                            $id
                    ));
                ...
                if ($delpic2 == '1') {
                    $stmt5 = $db->prepare(
                            "SELECT pic2 FROM shopping WHERE id=?");
                    $stmt5->execute(array(
                            $id
                    ));
                    $row5 = $stmt5->fetch();
                    ...
                    $stmt6 = $db->prepare(
                            "UPDATE shopping SET pic2=? WHERE id=?");
                    $stmt6->execute(array(
                            '0',
                            $id
                    ));
                ...
                $stmt7 = $db->prepare(
                        "UPDATE shopping SET title=?,content=?,showDate=? WHERE id=?");
                $stmt7->execute(array(
                        $title,
                        $content,
                        $showDate,
...
$stmt10 = $db->prepare(
        "SELECT * FROM shopping WHERE showDate BETWEEN $tstart AND $tend ORDER BY updated DESC");
$stmt10->execute();
while ($row10 = $stmt10->fetch()) {
    ...
    if ($loggedin == "1") {
        ...
        $thisYear = date("Y");
        $substmt = $db->prepare(
                "SELECT showDate FROM shopping ORDER BY showDate LIMIT 1");
        $substmt->execute();

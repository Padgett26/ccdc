...
$stmt2->execute ();
$row2 = $stmt2->fetch ();
$homeText = nl2br ( make_links_clickable ( html_entity_decode ( $row2 ['content'], ENT_QUOTES ) ) );

if ($loggedin == "1") {
	echo "<div><form action='index.php' method='post'><textarea name='homeText' rows='7' cols='42'>" . $row2 ['content'] . "</textarea><div style='text-align:right'><input type='submit' value=' Upload ' /></div></form></div>";
} else {
...
if ($loggedin == "1") {
	...
	if (isset ( $_FILES ['image2'] ['tmp_name'] ) && ($_FILES ['image2'] ['size'] > 1000)) {
		...
		if ($width != null && $height != null) {
			...
			processPic ( "explore", $picname2, "400", "400", $tmpFile, $imageType );
		}
	}
	...
	if (filter_input ( INPUT_POST, 'postnote', FILTER_SANITIZE_STRING )) {
		...
		if ($id == "new") {
			$stmt2 = $db->prepare ( "INSERT INTO explore VALUES" . "(NULL,?,?,NULL,?,?,'0','0')" );
			$stmt2->execute ( array (
					$title,
					$content,
					$picname1,
			...
			if ($delpost == "1") {
				...
				if ($delpic1 == '1') {
					...
					$stmt4 = $db->prepare ( "UPDATE explore SET pic1=? WHERE id=?" );
					$stmt4->execute ( array (
							'0',
							$id
					) );
				...
				if ($delpic2 == '1') {
					...
					$stmt6 = $db->prepare ( "UPDATE explore SET pic2=? WHERE id=?" );
					$stmt6->execute ( array (
							'0',
							$id
					) );
				}
				$stmt7 = $db->prepare ( "UPDATE explore SET title=?,content=? WHERE id=?" );
				$stmt7->execute ( array (
						$title,
						$content,
						$id
				...
				if ($picname1 != '0') {
					$stmt8 = $db->prepare ( "UPDATE explore SET pic1=? WHERE id=?" );
					$stmt8->execute ( array (
							$picname1,
							$id
					) );
				...
				if ($picname2 != '0') {
					$stmt9 = $db->prepare ( "UPDATE explore SET pic2=? WHERE id=?" );
					$stmt9->execute ( array (
							$picname2,
							$id
					) );
...
while ( $row10 = $stmt10->fetch () ) {
	$id = $row10 ['id'];
	$title = $row10 ['title'];
	$content = make_links_clickable ( html_entity_decode ( $row10 ['content'], ENT_QUOTES ) );
	$c1 = str_replace ( "\n", '<br>', $content );
	$c = explode ( "<br>", $c1 );
	$pic1 = $row10 ['pic1'];
	$pic2 = $row10 ['pic2'];
	...
	if ($loggedin == "1") {
		...
		echo "<div style='text-align:justify; font-family:sans-serif; padding:10px;'>" . $c [0] . "<br><br>";

...
if ($loggedin == "1") {
    ...
    if (isset($_FILES['image1']['tmp_name']) &&
        ...
        if ($width != null && $height != null) {
            ...
            processPic("$domain/image/commercial", $picname1 . "." . $imageType,
                    $tmpFile, 400, 150);
        }
    }
    ...
    if (isset($_FILES['image2']['tmp_name']) &&
        ...
        if ($width != null && $height != null) {
            ...
            processPic("$domain/image/commercial", $picname2 . "." . $imageType,
                    $tmpFile, 400, 150);
        }
    }
    ...
    if (filter_input(INPUT_POST, 'postnote', FILTER_SANITIZE_STRING)) {
        ...
        if ($id == "new") {
            ...
            if ($delpost == "1") {
                ...
                if ($delpic1 == '1') {
                    $stmt3 = $db->prepare(
                            "SELECT pic1 FROM commercial WHERE id=?");
                    $stmt3->execute(array(
                            $id
                    ));
                    $row3 = $stmt3->fetch();
                    ...
                    $stmt4 = $db->prepare(
                            "UPDATE commercial SET pic1=? WHERE id=?");
                    $stmt4->execute(array(
                            '0',
                            $id
                    ));
                ...
                if ($delpic2 == '1') {
                    $stmt5 = $db->prepare(
                            "SELECT pic2 FROM commercial WHERE id=?");
                    $stmt5->execute(array(
                            $id
                    ));
                    $row5 = $stmt5->fetch();
                    ...
                    $stmt6 = $db->prepare(
                            "UPDATE commercial SET pic2=? WHERE id=?");
                    $stmt6->execute(array(
                            '0',
                            $id
                    ));
                ...
                $stmt7 = $db->prepare(
                        "UPDATE commercial SET title=?,content=? WHERE id=?");
                $stmt7->execute(array(
                        $title,
                        $content,
                        $id

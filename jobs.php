...
if ($loggedin == "1") {
    ...
    if (isset($_FILES['image1']['tmp_name']) &&
        ...
        if ($width != null && $height != null) {
            ...
            processPic("$domain/image/jobs", $picname1 . "." . $imageType,
                    $tmpFile, 400, 150);
        }
    }
    ...
    if (isset($_FILES['image2']['tmp_name']) &&
        ...
        if ($width != null && $height != null) {
            ...
            processPic("$domain/image/jobs", $picname2 . "." . $imageType,
                    $tmpFile, 400, 150);
        }
    }
    ...
    if (filter_input(INPUT_POST, 'postnote', FILTER_SANITIZE_STRING)) {
        ...
        if ($id == "new") {
            $stmt2 = $db->prepare(
                    "INSERT INTO jobs VALUES" . "(NULL,?,?,NULL,?,?,'0','0')");
            $stmt2->execute(array(
                    $title,
                    $content,
                    $picname1,
            ...
            if ($delpost == "1") {
                ...
                $stmt7 = $db->prepare(
                        "UPDATE jobs SET title=?,content=? WHERE id=?");
                $stmt7->execute(array(
                        $title,
                        $content,
                        $id

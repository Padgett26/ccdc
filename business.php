...
$stmt2 = $db->prepare ( "SELECT id,category FROM busiCategories ORDER BY category" );
$stmt2->execute ();
...
if ($loggedin == "1") {
	if (filter_input ( INPUT_POST, 'confdelbusi', FILTER_SANITIZE_NUMBER_INT )) {
		$did = filter_input ( INPUT_POST, 'confdelbusi', FILTER_SANITIZE_NUMBER_INT );
		$stmtPic = $db->prepare ( "SELECT picture, picExt FROM business WHERE id=?" );
	...
	if (filter_input ( INPUT_POST, 'businessup', FILTER_SANITIZE_STRING )) {
		...
		$delbusi = (filter_input ( INPUT_POST, 'delbusi', FILTER_SANITIZE_NUMBER_INT ) == "1") ? "1" : "0";
		...
		if ($delbusi == "1") {
			...
			if ($id == "new") {
				$stmt3 = $db->prepare ( "INSERT INTO business VALUES" . "(NULL,?,?,?,?,?,?,?,?,?,?,?,?,?,'0','0','0')" );
				$stmt3->execute ( array (
						$category,
						$name,
						$contact,
				...
				$stmt2 = $db->prepare ( "SELECT id FROM business WHERE category=? && name=? ORDER BY id DESC LIMIT 1" );
				$stmt2->execute ( array (
						$category,
						$name
				) );
				...
				if (isset ( $_FILES ['image'] ['tmp_name'] ) && $_FILES ['image'] ['size'] > 1000) {
					...
					if ($width != null && $height != null) {
						...
						processPic ( "business", $time, "800", "800", $tmpFile, $imageType );
						$pstmt1 = $db->prepare ( "UPDATE business SET picture=?, picExt=? WHERE id=?" );
				...
				$stmt = $db->prepare ( "UPDATE business SET category=?,name=?,contact=?,phone=?,fax=?,email=?,website=?,mailing=?,physical=?,city=?,state=?,zipCode=?,description=? WHERE id=?" );
				$stmt->execute ( array (
						$category,
						$name,
						$contact,
				...
				if (isset ( $_FILES ['image'] ['tmp_name'] ) && $_FILES ['image'] ['size'] > 1000) {
					...
					if ($width != null && $height != null) {
						$imageType = getPicType ( $_FILES ["image"] ["type"] );
						processPic ( "business", $time, "800", "800", $image, $imageType );
						$pstmt2 = $db->prepare ( "UPDATE business SET picture=?, picExt=? WHERE id=?" );
		...
		if ($delPic == "1") {
			...
			$stmt4 = $db->prepare ( "UPDATE business SET picture=?, picExt=? WHERE id=?" );
			$stmt4->execute ( array (
					'0',
					'xxx',
					$id
	...
	if (filter_input ( INPUT_POST, 'categories', FILTER_SANITIZE_NUMBER_INT ) == "1") {
		foreach ( $_POST as $key => $val ) {
			...
			if (preg_match ( "/^categoryNew$/", $key, $match )) {
				if ($val != "" && $val != " ") {
					$stmt7 = $db->prepare ( "INSERT INTO busiCategories VALUES(NULL,?)" );
					$stmt7->execute ( array (
							$val
					) );
				}
	...
	if (filter_input ( INPUT_POST, 'categorize', FILTER_SANITIZE_NUMBER_INT ) == "1") {
		foreach ( $_POST as $key => $val ) {
			if (preg_match ( "/^category([1-9][0-9]*)$/", $key, $match )) {
				$cId = $match [1];
				$stmt8 = $db->prepare ( "UPDATE business SET category=? WHERE id=?" );
				$stmt8->execute ( array (
						$val,
						$cId
				) );
            ...
            <table>
                <tr>
                    <td>Category <select name="category">
                            <?php
	$stmt9 = $db->prepare ( "SELECT id,category FROM busiCategories ORDER BY category" );
	$stmt9->execute ();
	...
	$stmt10 = $db->prepare ( "SELECT id,category FROM busiCategories ORDER BY category" );
	$stmt10->execute ();
	...
	if ($row11 [0] >= 1) {
		...
		$stmt12 = $db->prepare ( "SELECT id,name FROM business WHERE category='0'" );
		$stmt12->execute ();
		while ( $row12 = $stmt12->fetch () ) {
			...
			$stmt13 = $db->prepare ( "SELECT id,category FROM busiCategories ORDER BY category" );
			$stmt13->execute ();
...
<div id="listCategory" style="clear:both; margin-top:30px;">
    <?php
				$stmt14 = $db->prepare ( "SELECT category FROM business ORDER BY RAND() LIMIT 1" );
				$stmt14->execute ();
				$row14 = $stmt14->fetch ();
				...
				if ($row15) {
					...
					while ( $row16 = $substmt->fetch () ) {
						...
						$description = nl2br ( make_links_clickable ( html_entity_decode ( $row16 ['description'], ENT_QUOTES ) ) );

    ...
    if (filter_input(INPUT_POST, 'postnote', FILTER_SANITIZE_STRING)) {
        ...
        if ($id == "new") {
            ...
            if ($delpost == "1") {
                ...
                $stmt7 = $db->prepare(
                        "UPDATE news SET title=?,content=?,showDate=? WHERE id=?");
                $stmt7->execute(array(
                        $title,
                        $content,
                        $time,
...
$start = ($blogYear == date("Y")) ? ($time - 31536000) : mktime(0, 0, 0, 1, 1,
        $blogYear);
$end = ($blogYear == date("Y")) ? $time : mktime(23, 59, 59, 12, 31, $blogYear);
$stmt10 = $db->prepare(
        "SELECT * FROM news WHERE showDate >= ? && showDate <= ? ORDER BY showDate DESC");

...
if ($loggedin == "1") {
    ...
    if (isset($_FILES['image1']['tmp_name']) &&
        ...
        if ($width != null && $height != null) {
            ...
            processPic("$domain/image/lodging", $picname1 . "." . $imageType,
                    $tmpFile, 400, 150);
        }
    }
    ...
    if (isset($_FILES['image2']['tmp_name']) &&
        ...
        if ($width != null && $height != null) {
            ...
            processPic("$domain/image/lodging", $picname2 . "." . $imageType,
                    $tmpFile, 400, 150);
        }
    }
    ...
    if (filter_input(INPUT_POST, 'postnote', FILTER_SANITIZE_STRING)) {
        ...
        if ($id == "new") {
            $stmt2 = $db->prepare(
                    "INSERT INTO lodging VALUES" . "(NULL,?,?,NULL,?,?,'0','0')");
            $stmt2->execute(array(
                    $title,
                    $content,
                    $picname1,
            ...
            if ($delpost == "1") {
                ...
                if ($delpic1 == '1') {
                    ...
                    $stmt4 = $db->prepare(
                            "UPDATE lodging SET pic1=? WHERE id=?");
                    $stmt4->execute(array(
                            '0',
                            $id
                    ));
                ...
                if ($delpic2 == '1') {
                    ...
                    $stmt6 = $db->prepare(
                            "UPDATE lodging SET pic2=? WHERE id=?");
                    $stmt6->execute(array(
                            '0',
                            $id
                    ));
                ...
                $stmt7 = $db->prepare(
                        "UPDATE lodging SET title=?,content=? WHERE id=?");
                $stmt7->execute(array(
                        $title,
                        $content,
                        $id

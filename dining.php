...
if ($loggedin == "1") {
	if (filter_input ( INPUT_POST, 'confdelpost', FILTER_SANITIZE_NUMBER_INT )) {
		...
		$stmt1 = $db->prepare ( "DELETE FROM dining WHERE id=?" );
		$stmt1->execute ( array (
				$id
		) );
		echo "Post deleted...";
	...
	if (isset ( $_FILES ['image1'] ['tmp_name'] ) && ($_FILES ['image1'] ['size'] > 1000)) {
		...
		if ($width != null && $height != null) {
			...
			processPic ( "dining", $picname1, "400", "400", $tmpFile, $imageType );
		}
	}
	...
	if (isset ( $_FILES ['image2'] ['tmp_name'] ) && ($_FILES ['image2'] ['size'] > 1000)) {
		...
		if ($width != null && $height != null) {
			...
			processPic ( "dining", $picname2, "400", "400", $tmpFile, $imageType );
		}
	}
	...
	if (filter_input ( INPUT_POST, 'postnote', FILTER_SANITIZE_STRING )) {
		...
		if ($id == "new") {
			$stmt2 = $db->prepare ( "INSERT INTO dining VALUES" . "(NULL,?,?,NULL,?,?,'0','0')" );
			$stmt2->execute ( array (
					$title,
					$content,
					$picname1,
			...
			if ($delpost == "1") {
				echo "Are you sure you want to delete this post? <form action='dining.php' method='post'><input type='hidden' name='confdelpost' value='$id' /><input type='submit' value=' YES ' /></form> <form action='dining.php' method='post'><input type='submit' value=' NO ' /></form>";
			} else {
				if ($delpic1 == '1') {
					$stmt3 = $db->prepare ( "SELECT pic1 FROM dining WHERE id=?" );
					$stmt3->execute ( array (
							$id
					) );
					$row3 = $stmt3->fetch ();
					$delpicname1 = $row3 ['pic1'];
					if (file_exists ( "image/dining/$delpicname1.jpg" )) {
						unlink ( "image/dining/$delpicname1.jpg" );
					}
					$stmt4 = $db->prepare ( "UPDATE dining SET pic1=? WHERE id=?" );
					$stmt4->execute ( array (
							'0',
							$id
					) );
				...
				if ($delpic2 == '1') {
					$stmt5 = $db->prepare ( "SELECT pic2 FROM dining WHERE id=?" );
					$stmt5->execute ( array (
							$id
					) );
					$row5 = $stmt5->fetch ();
					$delpicname2 = $row5 ['pic2'];
					if (file_exists ( "image/dining/$delpicname2.jpg" )) {
						unlink ( "image/dining/$delpicname2.jpg" );
					}
					$stmt6 = $db->prepare ( "UPDATE dining SET pic2=? WHERE id=?" );
					$stmt6->execute ( array (
							'0',
							$id
					) );
				}
				$stmt7 = $db->prepare ( "UPDATE dining SET title=?,content=? WHERE id=?" );
				$stmt7->execute ( array (
						$title,
						$content,
						$id
				...
				if ($picname1 != '0') {
					$stmt8 = $db->prepare ( "UPDATE dining SET pic1=? WHERE id=?" );
					$stmt8->execute ( array (
							$picname1,
							$id
					) );
				...
				if ($picname2 != '0') {
					$stmt9 = $db->prepare ( "UPDATE dining SET pic2=? WHERE id=?" );
					$stmt9->execute ( array (
							$picname2,
							$id
					) );
	...
	echo "<form action='dining.php' method='post' enctype='multipart/form-data'>\n";
...
$stmt10 = $db->prepare ( "SELECT * FROM dining ORDER BY RAND()" );
$stmt10->execute ();
while ( $row10 = $stmt10->fetch () ) {
	...
	if ($loggedin == "1") {
		...
		echo "Picture 1:<br />";
		if (file_exists ( "image/dining/$pic1.jpg" )) {
			echo "<img src='image/dining/$pic1.jpg' alt='' /><br /><input type='checkbox' name='delpic1' value='1' /> Delete this pic<br />";
		}
		echo "<input type='file' name='image1' /><input type='hidden' name='picname1' value='" . $time . "1' /><br /><br />";
		echo "Picture 2:<br />";
		if (file_exists ( "image/dining/$pic2.jpg" )) {
			echo "<img src='image/dining/$pic2.jpg' alt='' /><br /><input type='checkbox' name='delpic2' value='1' /> Delete this pic<br />";
		}
		...
		if (file_exists ( "image/dining/$pic1.jpg" )) {
			echo "<img src='image/dining/$pic1.jpg' alt='' style='float:right; margin:10px; max-width:300px;' />";
		}
		echo "<div style='text-align:center; font-family:sans-serif; padding:10px;'>$content</div>";
		if (file_exists ( "image/dining/$pic2.jpg" )) {
			echo "<img src='image/dining/$pic2.jpg' alt='' style='margin:10px 0px;' />";
		}
